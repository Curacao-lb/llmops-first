...
class AppConfigService(BaseService):
    ...
    def get_draft_app_config(self, app: App) -> dict[str, Any]:
        ...
        agents, validate_agents = self._process_and_validate_agents(
            draft_app_config.agents
        )
    ...
    def _process_and_validate_workflows(
        ...
        # 校验工作流配置列表，如果引用了不存在/被删除的工作流，则需要剔除数据并更新，同时获取工作流的额外信息
        workflows = []
        workflow_records = (
            self.db.session.query(Workflow)
            .filter(
        ...
        for workflow_id in validate_workflows:
            workflow = workflow_dict[str(workflow_id)]
            workflows.append(
                {
                    "id": str(workflow.id),
                    "name": workflow.name,
    ...
    def _process_and_validate_agents(
        self, origin_agents: list[UUID]
    ) -> tuple[list[dict], list[UUID]]:
        agents = []
        agent_records = (
            self.db.session.query(App)
            .filter(App.id.in_(origin_agents), App.status == AppStatus.PUBLISHED)
            .all()
        )
        agent_dict = {
            str(agent_record.id): agent_record for agent_record in agent_records
        }
        agent_sets = set(agent_dict.keys())

        # 计算存在的工作流id列表，为了保留原始顺序，使用列表循环的方式来判断
        validate_agents = [
            agent_id for agent_id in origin_agents if agent_id in agent_sets
        ]
