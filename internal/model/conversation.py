...
class Message(BaseModel):
    ...
    def conversation(self) -> Conversation:
        """只读属性，返回该消息对应的会话记录"""
        return cast(
            Conversation, db.session.query(Conversation).get(self.conversation_id)
        )
...
class MessageAgentThought(BaseModel):
    ...
    # 推理步骤关联信息
    app_id: Mapped[uuid.UUID] = mapped_column(UUID, nullable=False)  # 关联的应用id
    conversation_id: Mapped[uuid.UUID] = mapped_column(
        UUID, nullable=False
    )  # 关联的会话id
    message_id: Mapped[uuid.UUID] = mapped_column(UUID, nullable=False)  # 关联的消息id
    invoke_from: Mapped[str] = mapped_column(
        String(255),
        nullable=False,
        server_default=text("''::character varying"),
