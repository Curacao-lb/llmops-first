...
class WebAppHandler:
    ...
    def web_app_chat(self, token: str):
        ...
        response = self.web_app_service.web_app_chat(
            token, req, cast(Account, current_user)
        )
    ...
    def stop_web_app_chat(self, token: str, task_id: UUID):
        ...
        self.web_app_service.stop_web_app_chat(
            token, task_id, cast(Account, current_user)
        )
    ...
    def get_conversations(self, token: str):
        ...
        conversations = self.web_app_service.get_conversations(
            token, req.is_pinned.data, cast(Account, current_user)
        )
