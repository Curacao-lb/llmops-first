...
class WorkflowHandler:
    ...
    def delete_workflow(self, workflow_id: UUID):
        """根据传递的工作流id删除指定的工作流"""
        self.workflow_service.delete_workflow(workflow_id, cast(Account, current_user))
    ...
    def update_workflow(self, workflow_id: UUID):
        ...
        self.workflow_service.update_workflow(
            workflow_id, cast(Account, current_user), **req.data
        )
    ...
    def get_workflow(self, workflow_id: UUID):
        ...
        workflow = self.workflow_service.get_workflow(
            workflow_id, cast(Account, current_user)
        )
    ...
    def get_workflows_with_page(self):
        ...
        return success_json(
            PageModel(list=cast(list, resp.dump(workflows)), paginator=paginator)
        )
    ...
    def update_draft_graph(self, workflow_id: UUID):
        ...
        self.workflow_service.update_draft_graph(
            workflow_id, draft_graph_dict, cast(Account, current_user)
        )
    ...
    def get_draft_graph(self, workflow_id: UUID):
        ...
        draft_graph = self.workflow_service.get_draft_graph(
            workflow_id, cast(Account, current_user)
        )
    ...
    def debug_workflow(self, workflow_id: UUID):
        ...
        response = self.workflow_service.debug_workflow(
            workflow_id, inputs, cast(Account, current_user)
        )
    ...
    def publish_workflow(self, workflow_id: UUID):
        """根据传递的工作流id发布指定的工作流"""
        self.workflow_service.publish_workflow(workflow_id, cast(Account, current_user))
        return success_message("发布工作流成功")

    @login_required
    def cancel_publish_workflow(self, workflow_id: UUID):
        """根据传递的工作流id取消发布指定的工作流"""
        self.workflow_service.cancel_publish_workflow(
            workflow_id, cast(Account, current_user)
        )
