...
# 判断是否使用云端连接
if weaviate_api_key and "gcp.weaviate.cloud" in weaviate_url:
    # 云端连接
...
# 3.6 构建问题重写器（用于网络搜索前优化查询） 自然语言 变成 搜索引擎语言
...
def web_search(state: GraphState) -> Any:
    ...
    search_content = tavily_search.invoke({"query": question})
    documents.append(
        Document(
